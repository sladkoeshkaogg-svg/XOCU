...
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/sladkoeshkaogg-svg/XOCU/main/XOCU%20FAKELIBRORY"))()
...
do
    ...
    local function normalizeAssetId(value)
        ...
        local id = value:match("[?&]id=(%d+)") or value:match("rbxassetid://(%d+)") or value:match("^(%d+)$")
        return id and ("rbxassetid://" .. id) or value
    end
    function Visuals.applySkybox(n) local s=Visuals.SkyboxAssets[n]; if not s then return end; local sky=Lighting:FindFirstChildOfClass("Sky") or Instance.new("Sky",Lighting); sky.Name="Sky"; sky.SkyboxBk=normalizeAssetId(s.Bk); sky.SkyboxDn=normalizeAssetId(s.Dn); sky.SkyboxFt=normalizeAssetId(s.Ft); sky.SkyboxLf=normalizeAssetId(s.Lf); sky.SkyboxRt=normalizeAssetId(s.Rt); sky.SkyboxUp=normalizeAssetId(s.Up) end
